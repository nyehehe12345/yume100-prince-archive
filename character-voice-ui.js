class CharacterVoiceUI {
    constructor(options = {}) {
        ...
        // 注意：不再在构造函数中调用initializeExpressionIndex，因为它依赖于尚未设置的headFrames
    }
    ...
    initializeExpressionIndex() {
        ...
        const expressions = Object.keys(this.headFrames[this.currentAwaken]).filter(key => !key.includes('blink'));
        const defaultExpression = this.DEFAULT_EXPRESSIONS[this.currentAwaken];
        const defaultIndex = expressions.indexOf(defaultExpression);
        // 如果找到默认表情，使用其索引，否则使用0
        this.currentExpressionIndex = defaultIndex !== -1 ? defaultIndex : 0;
    }
    ...
    updateExpression() {
        ...
        if (this.currentExpressionIndex < 0) {
            this.currentExpressionIndex = expressions.length - 1;
        } else if (this.currentExpressionIndex >= expressions.length) {
            this.currentExpressionIndex = 0;
        }
        
        // 更新当前表情
        this.currentExpression = expressions[this.currentExpressionIndex];
...
// 创建全局实例
const characterVoiceUI = new CharacterVoiceUI();